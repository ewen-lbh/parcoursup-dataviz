...
for date, wishes in by_date.items():
    for wish in wishes:
        ...
        if key not in by_wish.keys():
            by_wish[key] = []
        computed_data = {}
        if wish['is_internat'] and all([ wish['internat'][key] is not None for key in ['condition_group_waitlist_rank', 'condition_rank', 'rank', 'group_waitlist_rank']]):
            computed_data = {
                'internat_group_diff': wish['internat']['condition_group_waitlist_rank'] - wish['internat']['group_waitlist_rank'],
                'internat_rank_diff': wish['internat']['condition_rank'] - wish['internat']['rank']
            }
        by_wish[key].append(
            {
                **wish,
...
for wish_id, wish in by_wish.items():
    ...
    if not is_internat:
        ...
        axs[idx].plot(data('date'), data('internat', 'condition_rank'), color="black", ls="--")
        axs[idx].legend(('Place dans le groupe', 'Condition pour rentrer', 'Classement', 'Condition pour rentrer'))
